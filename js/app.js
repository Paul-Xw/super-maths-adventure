// v6.0.7 force clear stale generated/cached questions
if(!localStorage.getItem('sma_v607_force_clear_done')){
  localStorage.setItem('sma_v607_force_clear_done','1');
  Object.keys(localStorage).filter(k=>k.startsWith('cache_')).forEach(k=>localStorage.removeItem(k));
}
// v6.0.7 cache killer
if(!localStorage.getItem('sma_v607_cache_killer_done')){
  localStorage.setItem('sma_v607_cache_killer_done','1');
...
async function startMission(mode){
  console.log('SMA v6.0.7 startMission',mode,currentWorld.slug);
...
function next(m,ok){
  ...
  const isLast = !state.qs || state.q >= state.qs.length - 1;
  const delay = isLast ? 450 : (ok ? 350 : 650);
  setTimeout(()=>{
    if(state.completed)return;
    state.q++;
    state.isAnswering=false;
    if(isLast || !state.qs || state.q>=state.qs.length){
      return nextLoop();
    }
    renderQuestion();
  },delay);
