...
function celebrateCorrect(){
  ...
  e.className='celebrate-correct';
  e.innerHTML=`<div class="celebrate-card">
    <div class="celebrate-icon">🌟</div>
    <div class="celebrate-title">Correct!</div>
    <div class="celebrate-sub">Great job! +10</div>
  </div>`;
...
function correct(){
  ...
  pop('✅ +10');
  celebrateCorrect();
  next(`<div class="p-4 rounded-2xl bg-green-100 font-black text-green-800">✅ Correct! Great job!</div>`,true);
}
...
function next(m,ok){
  ...
  const delay = isLast ? 1600 : (ok ? 1400 : 850);
  setTimeout(()=>{
    state.q++;
    state.isAnswering=false;
...
async function complete(){
  ...
  const star=calcStars(state.correct,total);
  ...
  shell(`<main class="glass rounded-[2rem] p-8 max-w-2xl mx-auto text-center">
    ...
    <h2 class="text-5xl font-black">Level Complete!</h2>
    <p class="mt-3 text-xl font-bold">${currentWorld.topic}</p>
    <p class="mt-2">Score ${state.score} • Correct ${state.correct}/${total}</p>
    <p class="mt-2 text-2xl font-black">Earned 🪙 ${coins}</p>
    <div class="grid md:grid-cols-2 gap-3 mt-7">
      <button id="playAgain" class="btn py-4 px-8 rounded-2xl bg-cyan-500 text-white font-black">Play Again</button>
      <button id="back" class="btn py-4 px-8 rounded-2xl bg-slate-900 text-white font-black">World Scenes</button>
    </div>
  </main>`);
  const again=document.getElementById('playAgain');
  const back=document.getElementById('back');
  if(again)again.onclick=()=>{state.finishing=false;openScene(state.idx)};
  if(back)back.onclick=()=>{state.finishing=false;renderWorldScenes()};
}
