// v6.0.2 cache killer
if(!localStorage.getItem('sma_v602_cache_killer_done')){
  localStorage.setItem('sma_v602_cache_killer_done','1');
...
function openScene(i){state.idx=i;state.score=0;state.corfunction renderStory(){let s=scenes[state.idx];shell(`<main class="glass rounded-[2rem] p-8 max-w-5xl mx-auto"><h2 class="text-5xl font-black">${s.name}</h2><div class="grid md:grid-cols-4 gap-2 mt-5">${['Learn','Example','Easy','Mini','Medium','Boss','Review'].map(x=>`<div class="p-3 rounded-xl bg-cyan-50 font-black">${x}</div>`).join('')}</div><button id="go" onclick="window.smaStartMission&&window.smaStartMission()" class="btn mt-6 px-7 py-4 rounded-2xl bg-cyan-500 text-white font-black">Start</button></main>`);const go=document.getElementById('go');if(go)go.onclick=()=>startMission('easy')}
mentById('go').onclick=()=>startMission('easy')}
async function startMission(mode){
  console.log('SMA v6.0.2 startMission',mode,currentWorld.slug);
...
function next(m,ok){document.getElementById('fb').innerHTML=m;setTimeout(()=>{state.q++;if(state.q>=state.qs.length)return nextLoop();renderQuestion()},ok?450:750)}
